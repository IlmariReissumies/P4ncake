...
Ancestors
  panLang panProps panSem compiler p4 p4_bigstep p4_exec_sem
Libs
  wordsLib
...
Type state = 
    <| locals      : varname |-> 'a v
     ; globals     : varname |-> 'a v
e     ; code        : funname |-> ((varname # shape) list # ('a panLang$prog))
...
Theorem compile_exp_correct:
  (∀ e uninit (scope_lists:scope_list) e' (n:num) m pan_e s.
    bigstep_e_exec uninit (scope_lists:scope_list) (INL (e)) (n:num) = SOME $ (INL e', m) ∧
    is_v e' ∧ compile_exp e = SOME pan_e ∧ state_rel scope_lists s ⇒
    ∃ v. eval s pan_e = SOME v ∧
         v_rel e' v ) ∧
  ∀ es uninit (scope_lists:scope_list) es' (n:num) m pan_es s.
